...
import { compileVectorSearch } from "../Modules/Plugin/HotKey";
import WatcherService, {
  CompileContext,
} from "../Scripts/Plugin/WatcherService";
...
import { buildVectorIndex, PageIndex } from "../Modules/Plugin/vectorIndex";
...
export function summarizeElementsForAI(hits, currentContext, maxLabel = 60) {
  return hits.map((r) => {
    ...
    const parentText = el?.parentElement?.innerText || undefined;
...
export function extractSelectedTool(text) {
  ...
  //     ```json
  // {
  //   "selected_tool": "Summarize Text",
  //   "runner_ups": [],
  //   "Use_summarizer": "True",
  //   "Vector_Search": "body",
  //   "Vector_SearchAmount": 1000
  // }
  // ```

  const match = text.match(/"selected_tool"\s*:\s*"([^"]+)"/);
  const match2 = text.match(/"Use_summarizer"\s*:\s*"([^"]+)"/);
  // Extract Vector Search and Vector Search amount using regex
  let match3 = text.match(/"Vector_Search"\s*:\s*"([^"]+)"/);
  let match4 = text.match(/"Vector_SearchAmount"\s*:\s*"([^"]+)"/);

  if (match) {
    // if not match 3 set default
    if (match3 == null) {
      match3 = "  ";
    }
    ...
    // Convert match4 to numbe

    return [match2[1].toLowerCase().trim(), match[1], match3[1], Number(match4[1])]
  }
}
...
export function getToolInfo(tools, toolName) {
  ...
  const found = tools.find(
    (tool) => tool.title.toLowerCase().trim() === toolName.toLowerCase().trim()
  );
...
-
  ...
  Now produce the reasoning and final JSON result as instructed above.`;
      ...
      try {
        ...
        const tool_info = getToolInfo(userPrompts, selected_tool[1]);
        console.log(`${session.inputUsage}/${session.inputQuota}`);


        const vector_search_results = await compileVectorSearch(
          selected_tool[2],
          selected_tool[3],
          currentContext
        );

        const summarized_elements = summarizeElementsForAI(
          vector_search_results,
          currentContext
        );

        console.log("Summarized elements for AI:", summarized_elements);
...
Your output will be injected directly into html so you can use tags like <b>, <i>, <br>, etc for formatting where appropriate.
...
--------
RELEVANT WEBSITE ELEMENTS:
${JSON.stringify(summarized_elements, null, 2)}
