...
function translateElement(el) {
  ...
  // Non-elements other than text: ignore
  if (el.nodeType !== Node.ELEMENT_NODE) return '';

  // Skip non-content elements
  if (SKIP_TAGS.has(el.tagName)) return '';

  const uid = elementIdentifierFrom(el);

  // Recursively render all children in order
  let inner = '';
  for (let i = 0; i < el.childNodes.length; i++) {
    inner += translateElement(el.childNodes[i]);
  }

  // Normalize spaces at the element boundary
  inner = inner.replace(/\s{2,}/g, ' ').trim();

  // Always wrap element with identifier markers
  return `<${uid}: ${inner} >`;
}
